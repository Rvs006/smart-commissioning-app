...
- A repository-wide documentation index now separates current guidance,
  candidate release material, versioned procedures, historical records, and
  generated files. Current field and Phase 5 guidance reflects the v0.1.29
  scale-aware capture contract.
...
- **IP discovery Stop is honest.** Cutting a host's port batch short no longer
  ...
  probed.
- **The run-progress event stream tolerates store interruptions:** it reads the
  run off the event loop, so a long inline-run write
...
- **A directed Who-Is to the addresses in your `bacnet_register` import.** The
  ...
  **A device that stays silent is reported amber, as "expected but did not
  answer" - never as offline.** That deliberate wording preserves uncertainty: BACnet
...
- **Repo-wide over-engineering cleanup (~1,750 lines net, behavior-neutral).**
  ...
  `backend/tests/harness.py` collapses the database-test boilerplate duplicated
