...
For the MQTT/UDMI field check, follow [the portable field checklist](docs/field-quickstart.md#mqttudmi-field-check-v0124-or-later) after startup. The **latest release** link above is the single supported download location; confirm that its tag and bundled `README_FIRST.txt` both name v0.1.42 before using this checklist. The v0.1.42 protocol recovery has its own [field validation guide](docs/v0.1.42-protocol-recovery-validation.md), including the IP-register, BACnet/IP, and MQTT checks.
