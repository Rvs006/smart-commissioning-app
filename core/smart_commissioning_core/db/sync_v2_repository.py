...
def sync_receipt_id(
    ...
    return sha256(
        "\0".join((credential_id, bundle_id, item_id, receipt_class)).encode("utf-8")
    ).hexdigest()
        ...
        with self._session_factory.begin() as session:
            ...
            if existing_run is not None:
                ...
                if artifact is not None and (
                    str(artifact["artifact_sha256"]) != artifact_sha256
                ):
            ...
            receipt_id = sync_receipt_id(
                credential_id, bundle_id, item_id, receipt_class
            )
            self._add_receipt(
                session,
    ...
    def _existing_item_is_identical(
        ...
        *,
        ...
        if existing_run.job_type == "report_generation":
            ...
            normalized_run["parameters"] = {
                "context_sha256": existing_seal.context_sha256
            }
        ...
        if artifact_sha256 is None:
            return False
        if (
            incoming_manifest.get("artifact_sha256") != artifact_sha256
            or not isinstance(incoming_manifest.get("byte_size"), int)
        ):
            return False
        ...
        return (
            ...
            and existing_artifact.renderer_version
            == incoming_manifest.get("renderer_version")
            and existing_artifact.origin == incoming_manifest.get("origin")
            and existing_artifact.signing_key_id
            == incoming_manifest.get("signing_key_id")
        )
    ...
    def _backfill_identical_evidence(
        ...
        *,
        ...
        if (
            ...
            and session.get(RunExecutionContext, run_id) is None
        ):
            session.add(
                RunExecutionContext(
                    run_id=run_id,
        ...
        with self._session_factory.begin() as session:
            for receipt in receipts:
                ...
                if receipt.get("run_id") != run_id:
                    continue
                if receipt.get("acknowledged") is not (
                    receipt_class in ACKNOWLEDGED_RECEIPT_CLASSES
                ):
                    continue
                ...
                if state is None:
                    ...
                    state.item_sha256 != str(descriptor["item_sha256"])
                    or state.result_sha256 != str(descriptor["result_sha256"])
                ):
                    raise RuntimeError(
                        f"Local sealed evidence changed for run {run_id}; receipt not applied."
                    )
