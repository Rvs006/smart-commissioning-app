...
> existing `frontend/dist`). For a same-organization portable that can approve
> the locally installed Nmap once from IP Scanner, add `-InternalOperatorNmap`.
> That switch does not package, download, or install Nmap or Npcap. The freeze + clean-box launch are still build-box /
