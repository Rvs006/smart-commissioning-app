...
| Area | Status | Documents |
| --- | --- | --- |
| Internal portable release | v0.1.55 (latest) | [Release notes](release-notes-v0.1.55.md), [validation](release-validation-v0.1.55.md), and [rollback](migration-rollback-v0.1.55.md) |
| Hosted Docker | v0.1.55 (latest) | [Docker deployment and rollback](docker-deployment-rollback-v0.1.55.md) |
...
## v0.1.55 release record

v0.1.55 is the latest published release. It finishes the scanner marriage: the
embedded IP, BACnet, and MQTT scanners are reskinned to the SCT theme and wired so
a scan done in the tool lands as a real SCT run, with the saved configuration
pushed into the tool on open. The IP, BACnet, and MQTT sidecar modules present the
embedded scanner as one Scan surface. This release also binds a panel session to
its protocol (a cross-protocol write is refused), shows a Retry error instead of a
ready-looking panel when the session or sidecar is unavailable, and persists a
BACnet export's discovered points. It builds on the Advanced-panel reverse proxy
from v0.1.53 and v0.1.54 and adds no database migration.
