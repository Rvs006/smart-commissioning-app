...
> Historical lab script for v0.1.17. Use
> [phase5-onsite-validation.md](phase5-onsite-validation.md) for the current
> live-hardware gate.
...
**On this path that sentence confirms a healthy save.** (The previous version of the operator
...
| **BBMD Address** | The lab BBMD's **real IP**, e.g. `198.51.100.20` | Seeded value `192.0.2.20` is demo data and does not exist. **Bare IP only** - no `:port`. |
...
> `using foreign-device registration via BBMD 198.51.100.20:47808`
...
- `Live bacpypes3 scan - foreign-device registration via BBMD 198.51.100.20` -
...
3. Path B: the results page will carry the engine's own sentence under
   **"Discovery complete - no BACnet devices responded"**, e.g.:
   ...
   > Registered with the BBMD at 198.51.100.20:47808, but no devices answered the Who-Is
...
**That list is your honest punch list.** Read Section 8 before treating any
item as "device offline"; each row is an observation that still needs classification.
...
> The BBMD at 198.51.100.20:47808 refused foreign-device registration (BVLL result code 3).
...
> No response from the BBMD at 198.51.100.20:47808 - it did not acknowledge foreign-device
...
`10.002s`, `10.13s`. Both values are expected.)
...
> UDP port 47808 on 198.51.100.50 is already in use by another program - usually another
...
> Cannot bind UDP port 47808 on 198.51.100.50 for a live BACnet scan (error 10049). Check
...
- **Amber "expected but did not answer" rows are inconclusive, not "device
  ...
  > directed Who-Is sent to 198.51.100.7. This is INCONCLUSIVE, not proof the device is
