# v0.1.50 migration and rollback

v0.1.50 retains the Alembic head `a6b7c8d9e0f1` and adds no database
...
API, worker, and frontend from the same v0.1.50 release, then confirm health,
the visible version, Brief and Learning content, worker heartbeat, one report
download, and the evidence manifest.

To roll back to v0.1.49, stop v0.1.50 after active work ends and restore the
recorded v0.1.49 EXE or immutable image digests. No Alembic downgrade is
...
v0.1.49 and v0.1.50 API or worker processes.
