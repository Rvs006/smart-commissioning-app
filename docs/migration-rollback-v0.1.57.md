# v0.1.57 migration and rollback

v0.1.57 retains the Alembic head `a6b7c8d9e0f1` and adds no database
migration. It changes the native-scanner screen layout and release identity
only. The
...
API, worker, and frontend from the same v0.1.57 release, then confirm health,
the visible version, the single-page native scanner screens, worker heartbeat,
one report download, and the evidence manifest.

To roll back to v0.1.56, stop v0.1.57 after active work ends and restore the
recorded v0.1.56 EXE or immutable image digests. No Alembic downgrade is
...
v0.1.56 and v0.1.57 API or worker processes.
