...
- ☐ UDMI validation against **live** state/pointset capture (not just the bundled fixture): silent-device detection, expected-units-vs-payload matching, schedule checks, and the pinned UDMI 1.5.2 schema-version/structural checks (strict RFC 3339 timestamps) behave on real payloads — including a register-driven run that fans out one expected asset per register row and fills the Results table with real per-asset/per-payload rows.
...
- ☐ **UDMI Workbench run time (PR #63)**: with the Setup-stage "Run time (seconds)" box **blank**, a register-driven live run captures until EVERY expected topic (distinct, across all register assets, wildcard-aware) has a payload — duplicates on one chatty topic must NOT end the window early — or the operator cancels. On the **worker** path `result_summary.capture_mode == "indefinite"`; the SAME blank request on the **inline/portable-exe** path is bounded to 240s and flags `indefinite_bounded_inline == true`. A typed positive number is honoured as an exact bounded window; non-numeric input (e.g. `45s`) is rejected with a validation error, not silently coerced. A partial capture must end `live_capture_timeout` with a `not_publishing` issue naming the specific missing topics — never a fake pass. Cancel mid-capture keeps the partial payloads and flips the run to `cancelled`.
