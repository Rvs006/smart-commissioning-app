...
- ☐ **UDMI Workbench run time and large-register capacity**: with the Setup-stage "Run time (seconds)" box **blank**, a register-driven live run captures until every required topic group has a payload or the operator stops the run. Duplicates on one chatty topic reuse that topic's slot. When the operator supplies no positive `max_messages`, the capacity is the larger of 500 or the concrete validation-filter count; the known 554-row register therefore receives at least 2,216 slots. Confirm the capture continues beyond retained message 500. On the **worker** path `result_summary.capture_mode == "indefinite"`, while the actor remains capped at 49 hours so the 48-hour capture backstop can finish first. The same blank request on the **synchronous inline** path (`INLINE_RUN_ASYNC=0`) is bounded to the default window and records `indefinite_bounded_inline == true`. A positive run time is an upper bound and may finish early when every required group reports. Explicit `0` and values such as `45s` are rejected. Broker or settings errors are terminal `failed`; an ordinary incomplete window is terminal `succeeded` under `udmi_validation_complete_with_silent_devices` with `live_capture_timeout` and a `not_publishing` issue. Cancellation keeps partial payloads and records terminal `cancelled`.
- ☐ **Wrong-topic and frozen-register evidence**: publish a registered test asset below a safely bounded but incorrect topic root. Confirm the asset remains observed, its payload content is validated, and the UI shows expected and actual topics. Generate all four reports from one scope. Excel must contain `Wrong Topic Assets` and `Annotated Input Register`; the ZIP must contain `wrong_topic_assets.json` and `annotated_input_register.json`. The annotated register must preserve all source rows and source columns before its review columns.
