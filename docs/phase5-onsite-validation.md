...
- ☐ **UDMI Workbench run time and large-register capacity**: import the 780-row legacy blank-applicability register and confirm it creates exactly 2,340 expected payload slots. During a finite run, publish expected traffic for at least 25 assets plus more than 500 distinct diagnostic topics. Expected-topic traffic must be observed even when its JSON is invalid; diagnostic truncation must be explicit and must not stop the window. The run may report external `window_elapsed` and `window_completed=true` only after the monotonic deadline evidence is true. Receiving every expected topic is never an early-success condition for a finite run. A primary topic or byte-cap exit, broker interruption, or Stop is incomplete and never `window_elapsed`. Stop is per-run, preserves partial evidence, is shown as **Stopped**, and a later run must still complete normally. Verify the documented 256 MiB primary latest-evidence budget and independent 2 MiB diagnostic budget before using a large register in the field. Explicit `0` and values such as `45s` are rejected.
