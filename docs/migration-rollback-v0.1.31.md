# v0.1.31 migration and rollback

v0.1.31 changes asset-topic observation and its release evidence. It adds no
database migration. The required Alembic head remains a7b8c9d0e1f2. The
...
- Windows portable: extract the v0.1.31 ZIP into a new directory and start the
  new executable. The app continues to use %LOCALAPPDATA%\SmartCommissioning.
- Hosted: deploy the three v0.1.31 digest references together, run the normal
...
and API health version must all identify v0.1.31 and the same source commit.
Renaming an older executable is not an upgrade.

## Rollback

Application rollback needs no Alembic downgrade because the schema did not
change. Stop v0.1.31, restore the recorded v0.1.30 executable or image digests,
