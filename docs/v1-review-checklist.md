...
| IP scanner should distinguish UDP/TCP and scan common ports if none specified. | Implemented | The core `ip_scan` engine parses port specifications with a common-port fallback; IP Discovery exposes editable port/protocol rows and sends the selected `port_specification` to the API. | Backend unit tests, frontend build |
