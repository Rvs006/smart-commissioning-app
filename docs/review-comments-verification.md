...
> Historical verification record for the original 24 review comments. File
> locations and line numbers may have moved. Current operating and field checks
> live in [docs/README.md](README.md). Since v0.1.29, register-driven UDMI
> capture uses the larger of 500 or its concrete validation-filter count when no
> positive message limit is supplied.
...
- frontend/src/features/workflow/ConfigurationPage.tsx:156 - "Certificate Expiry" is defined as `{ kind: "readonly" }`; editing and required-input validation do not apply.
...
**Verify on localhost:** 1) Start the backend API and frontend dev server (`npm run dev` in `frontend/`, default `http://127.0.0.1:5173`). 2) Open `http://127.0.0.1:5173/#/configuration`. 3) The Certificates & Keys section is expanded by default. 4) Confirm Certificate Expiry is a greyed read-only input with no required asterisk. The seeded `2027-05-20` date keeps the field neutral and shows the stored-expiry hint. 5) Store an expired CA or client certificate to turn the border and text red, or run the `flags an expired certificate-expiry field in red` frontend test, which seeds `2000-01-01`.

**Caveats / notes:** The backend parses the uploaded certificate's X.509 `notAfter` value and writes it into the read-only status field; the frontend compares that date with today. A fresh install stays neutral because the seed date is `2027-05-20`. The stale comment at ConfigurationPage.tsx:175-178 still describes PEM parsing as on-site work, although the backend now performs it. Private keys and unparseable PEM input leave the displayed expiry unchanged.
...
- `frontend/src/features/workflow/ConfigurationPage.tsx:70-139` - `TIMEZONE_OPTIONS` is a 69-entry IANA list starting with `UTC` and spanning each UTC-offset region, rather than Europe alone.
- `frontend/src/features/workflow/ConfigurationPage.tsx:167-169` - `fieldDefinitions.time.Timezone` uses a select backed by `TIMEZONE_OPTIONS`.
- `frontend/src/features/workflow/ConfigurationPage.tsx:726-741` - the `FieldControl` select branch preserves a stored value when it is absent from the list, then renders every known option.
- `frontend/src/features/workflow/ConfigurationPage.tsx:44` - the Time and NTP section starts collapsed, so the operator expands it before using the dropdown.
- `backend/app/services/configuration_service.py:76-79` - the backend stores the raw IANA zone name and round-trips values from any region.
- `frontend/src/features/workflow/ConfigurationPage.test.tsx:153-171` - the test opens Time and NTP, confirms a combobox, and checks UTC plus zones outside Europe.
...
- frontend/src/features/workflow/ConfigurationPage.tsx:594-602 - A backups-only field-note paragraph renders when the Backup & Restore section is expanded, stating the bundle 'is built from the app runtime (under the backend runtime directory by default) and written to a path chosen at backup time via the backup CLI's output option. The Backup Location field below records the intended target for operators; the app does not pick a host directory itself.' This directly answers in-app vs host-directory.
- frontend/src/features/workflow/ConfigurationPage.tsx:57-58 - sectionDescriptions.backups = 'Backup schedule, retention, encryption, storage location, and restore readiness.' is shown as section-copy (line 593) above the field note, flagging storage location as part of this section.
- backend/app/services/configuration_service.py:85-95 - DEFAULT_CONFIGURATION.backups includes a 'Backup Location' field defaulting to '/data/backups', so the field referenced by the clarifying note actually appears in the section's value list.
- frontend/src/features/workflow/ConfigurationPage.tsx:146-150 - fieldDefinitions.backups only overrides 'Encrypted Backups'/'Last Backup Status'/'Restore Action'; 'Backup Location' is absent, so it falls back to kind 'text' (line 611) and is an editable operator-specified target string, consistent with the note.
- backend/app/services/backup_service.py:136-178 - create_backup_bundle returns the bundle as in-memory zip bytes (no on-disk write location is chosen by the engine); the caller/CLI decides where to write, confirming the note's claim that the app does not pick a host directory itself.
- docs/review-comments-2026-06.csv:17 - The source review (review-mq9lv0g2) asks 'Where will the backup location be stored... Is it within the app or can a directory on the host laptop be specified?', matching the ask being verified.
...
- frontend/src/features/workflow/moduleData.ts:209 - The literal `warning` belongs to the data-validation route's unrendered `readiness` array. The `/udmi-validation` result state comes from a separate route and remains binary Pass/Fail.
...
**Caveat:** The Network number and IP address columns render in the live table, sample preview, and Selected Result Detail inspector. The backend device record still omits `network_number` and `ip_address`, so live rows show `-` while the sample preview shows populated values. The Address column continues to carry network location, for example `192.0.2.11:47808`.
...
- frontend/src/features/workflow/moduleData.ts:182-187 - the udmi-validation route's single runAction is labeled "Run UDMI Validation" (kind validation, runKind udmi, jobType udmi_validation); no 'Queue' wording remains.
- frontend/src/features/workflow/ModulePage.tsx:1006-1013 - the Run Controls action button text is computed as "Working..." while pending, else "Preview" (dry-run discovery), "Generate" (reports), else "Run" - so the UDMI action renders the button label "Run", never "Queue".
- frontend/src/features/workflow/ModulePage.tsx:1507-1516 - the secondary UDMI-only trigger in the Schedule and Payload Evidence card reads "Execute capture" (and "Executing..." while pending), not 'Queue'.
- frontend/src/features/workflow/moduleData.ts (grep for [Qq]ueue) - No matches found; the entire moduleData action source has zero 'Queue' occurrences.
- frontend/src/features/workflow/ModulePage.test.tsx:276-280 - regression test 'renames the discovery run action from Queue to Run' asserts findByText("Run IP Discovery"), and the same Run-prefix label convention is applied to the UDMI action.
- frontend/src/features/workflow/operatorData.ts:223-225 & DashboardPage.tsx:280 - the only remaining 'Queue' user-facing strings ('Queue evidence', 'Queue evidence pack', 'Report Queue') live on the homepage/dashboard reports flow, NOT on the /udmi-validation route.
...
- frontend/src/features/workflow/ModulePage.tsx:1507-1522 - Inside the udmi-validation "Schedule and Payload Evidence" section there is an Execute button ("Execute capture" / "Executing...") in an .execute-row, wired to onClick={() => runMutation.mutate(udmiRunActionIndex)} and gated to engineers.
...
- frontend/src/features/workflow/ModulePage.tsx:425-444, 2049-2071 - The validation run mutation builds udmi parameters via buildUdmiValidationParameters, sending state_topic/metadata_topic/pointset_topic, capture_seconds and use_live_broker to the API (startValidationRun, runKind "udmi").
- frontend/src/features/workflow/moduleData.ts:180-188 - The udmi-validation module defines a validation runActions entry (runKind "udmi", jobType "udmi_validation"); udmiRunActionIndex (ModulePage.tsx:535-540) resolves to it so the Execute button triggers the same run as Run Controls.
- core/smart_commissioning_core/udmi_validation.py:344-458 - _capture_live_payloads honours use_live_broker, builds the state/metadata/pointset topic list (_capture_topics), subscribes via live_capture, and stores captured payloads keyed by topic suffix (/state,/metadata,/pointset); returns broker_unreachable/timeout when no broker.
- worker/app/tasks.py:209-224 - validate_udmi_payloads dispatches process_udmi_validation_run with the real subscribe_and_capture live-capture path, so a real worker actually goes out and grabs the live payloads.
...
**Caveats:** This historical review recorded live broker capture as on-site untested. An open-ended capture stopped on Cancel or the `DEFAULT_MAX_MESSAGES` limit of 500, so it had no time deadline but retained a message cap. v0.1.29 replaces that fixed live-validation ceiling with register-driven retained capacity; see the status note at the top of this file. Empty live results remain empty.
...
**Verify on localhost:** 1) Start backend + frontend dev server. 2) Open http://127.0.0.1:5173/#/ip-scanner. 3) In the left 'Register Import' card, the import profile select defaults to 'ip register'; below 'Upload and validate' you see the 'Default import template' card. Click 'Download CSV' and 'Download XLSX' - browser saves ip_register_template.csv / .xlsx (server filename ip_register_default_template.*). 4) Open the file: row 1 is the required column headers (Project/site, System, Asset ID, Asset name, Expected IP address, Expected hostname, Expected services/ports) and row 2 is a realistic example (e.g. 192.0.2.117, 47808/udp, 443/tcp). 5) Scroll to the 'Import Templates for This Page' section lower on the page for the same XLSX/CSV buttons.
