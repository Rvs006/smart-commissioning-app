# v0.1.56 migration and rollback

v0.1.56 retains the Alembic head `a6b7c8d9e0f1` and adds no database
...
API, worker, and frontend from the same v0.1.56 release, then confirm health,
the visible version, Brief and Learning content, worker heartbeat, one report
download, and the evidence manifest.

To roll back to v0.1.53, stop v0.1.56 after active work ends and restore the
...
v0.1.53 and v0.1.56 API or worker processes.
