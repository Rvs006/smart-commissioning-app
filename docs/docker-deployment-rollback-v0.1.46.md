# v0.1.46 Docker deployment and rollback
...
If health startup rejects `SMART_COMMISSIONING_APP_VERSION`, replace the image
or compose value with the exact version recorded by its image label and frontend
build. Do not operate a backend stamped for another release.

For rollback, restore the recorded compatible v0.1.44 digest set with
`up -d --no-build`; restore the database backup at the pre-v0.1.46 head before
