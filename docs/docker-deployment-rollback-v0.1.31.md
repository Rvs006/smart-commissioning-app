# v0.1.31 Docker deployment and rollback

Deploy v0.1.31 only from the immutable references in
docker-image-evidence.json. API, worker, and frontend entries must share the
release commit and the v0.1.31 OCI version label.
...
5. Confirm API health, frontend version, worker heartbeat, an unfiltered
   observation run, and one report download before accepting the deployment.

## Roll back

Stop the application containers, restore the recorded v0.1.30 digest
