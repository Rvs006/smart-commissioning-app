...
| Jobs accepted but never progress (hosted) | Worker down, or Redis unreachable from the worker. | Check the worker log; confirm it shares `DATABASE_URL`/`REDIS_URL`. The API may report a "labelled local inline fallback" when the queue is unavailable. |
