# v0.1.49 migration and rollback

v0.1.49 retains the Alembic head `a6b7c8d9e0f1`; it adds no database migration.
It changes browser-side ownership of preview/live run observations and downloads.
The retained Sync v2 immutable-evidence head `a7b8c9d0e1f2`,
...
worker, and frontend from the same v0.1.49 release, then confirm health,
visible version, worker heartbeat, one report download, and the evidence
manifest.

To roll back to v0.1.48, stop v0.1.49 after active work ends and restore the
recorded v0.1.48 EXE or immutable image digests. No Alembic downgrade is
...
v0.1.48 and v0.1.49 API or worker processes.
