# v0.1.58 migration and rollback

v0.1.58 retains the Alembic head `a6b7c8d9e0f1` and adds no database
migration. It raises the built-in MQTT discovery topic ceiling and changes
release identity only. The
...
API, worker, and frontend from the same v0.1.58 release, then confirm health,
the visible version, one built-in MQTT discovery capture, worker heartbeat,
one report download, and the evidence manifest.

To roll back to v0.1.57, stop v0.1.58 after active work ends and restore the
recorded v0.1.57 EXE or immutable image digests. No Alembic downgrade is
...
v0.1.57 and v0.1.58 API or worker processes.
