# v0.1.38 Docker deployment and rollback
...
2. Set the three recorded digest references, `APP_VERSION=v0.1.38`, and the
   exact `SOURCE_COMMIT` from the hosted evidence.
3. Start the stack with `docker compose ... up -d --no-build`.
4. For inline execution, add `-f infra/docker-compose.inline.yml`. Confirm API
   health, frontend version, worker heartbeat, an approved run, and one report
   download.

## Rollback

Restore the previously recorded compatible v0.1.37 digest set and start with
