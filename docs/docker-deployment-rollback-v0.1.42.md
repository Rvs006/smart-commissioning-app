# v0.1.42 Docker deployment and rollback
...
For rollback, restore the recorded compatible v0.1.41 digest set with
`up -d --no-build`; restore the database backup at the pre-v0.1.42 head before
starting the older containers. Do not delete volumes containing evidence,
protected raw artifacts, or encrypted configuration. Store actual image names,
broker details, and secrets only in the private deployment record.
