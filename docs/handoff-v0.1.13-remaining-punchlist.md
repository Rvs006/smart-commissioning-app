...
Pete (the field engineer) did a walkthrough of the app on 2026-07-15 and left **24 notes**. Those
...
- **v0.1.13 — this release.** Everything from Pete's list that was **still not done** after v0.1.11
  and v0.1.12. Twelve items implemented, two deliberately deferred.

An audit ran first, checking every one of Pete's asks against the actual code on `main` + the
...
## 3. The 12 items implemented (each mapped to Pete's exact words)

### Reports

1. **Electracom headers/footers on reports** — Pete: *"Ensure reports are generated within
...
2. **Per-silo report content** — Pete: *"Reporting needs to be available per test silos."* The
...
3. **Don't fail the whole run on one silent device** — Pete: *"cant bin the whole validation if one
...
4. **RAG on the results page** — Pete: *"Green – online, publishing and UDMI compliant / Amber –
   online, publishing but not UDMI compliant / RED – offline and unable to check payloads."*
   Added the missing **amber** state and re-mapped the colours to exactly Pete's scheme: offline/not
...
5. **Nonpub schema template** — Pete: *"generate nonpub schema template to be added to the
...
6. **Compare broker scan against the template** — Pete: *"currently… seems to ignore the uploaded
...
7. **UDMI-style long timer** — Pete: *"needs the same timer we have in UDMI to allow us to run the
...
8. **MQTT inspector (was dead)** — Pete: *"inspector at bottom… currently does nothing… should dig
...
9. **Show non-responders** — Pete: *"IP scan silently drops unresponsive IP Addresses… needs to show
...
10. **Certs pill wrong after upload** — Pete: *"Certs and keys… hero showing not configured after
...
11. **Logging local-or-URL** — Pete: *"ability to log locally to machine, or to upload to URL."*
...
12. **Purge stale placeholder / Docker-era content** — Pete: *"Check documents available at top of
...
- **General nmap-style discovery pane** — Pete: *"last section doesn't bring much… better this is
  turned into a general nmap discovery section."* Deferred because the **substance of the feature is
  a curated port list that only Pete can supply** (top-20 / top-100), and building the pane before
...
- **Look-and-feel component extraction (Part B)** — Pete: *"Look and feel of each testing silo to be
...
**But the part Pete actually feels DID ship:** the **scroll-unbind** (look-and-feel Part A). The
tables no longer trap content inside a fixed-width scroll box — that was Pete's concrete complaint,
and it's a CSS-only change that landed in this release.

---

## 5. Open decisions for Pete (each shipped with a sensible default — one line to change)

Everything below already works with a default; these are Pete's calls, not blockers.
...
- **RAG "pass with notes" → amber.** Default shipped: a minor-only "pass with notes" device shows
  **amber**, and a publishing device with critical issues is **amber** (red is reserved for
  offline/not-publishing). If Pete wants minor-only to stay green, it's a one-line flip in
  `udmiVerdictTone`.
...
- **Report sign-offs.** Default shipped: A4 paper, "ELECTRACOM" text wordmark + run-id footer, no
  doc/revision-number field. Pete confirms whether that's ITP-sufficient (adding fields later
...
- **Log-upload wire contract.** Default shipped: a **multipart POST (field `file`) + Bearer token**
  — an implemented guess. If Pete's web endpoint expects something else, it's a one-function change.
...
5. When you next touch the older `docs/handoff-2026-07-15-pete-walkthrough.md`, correct a few stale
...
## 7. Honesty caveats that MUST reach the operator (Pete)

These are behaviours that are correct but surprising. Put them in the runbook / release notes.

- **Saved config snapshots don't auto-adopt new default VALUES.** If Pete has a saved configuration,
  changing a default in code does **not** rewrite his stored value. Fabricated *statuses*
  self-heal (the migration fixes them), but any real *value* Pete needs (e.g. the BACnet Foreign
...
- **Historical FAILED UDMI runs stay FAILED.** The "succeed with silent devices" fix (item 3) only
  affects **new** runs. Runs that already failed under the old rule remain failed — Pete must re-run
...
- **Re-download old reports after upgrading.** Report branding + inventory (items 1 and 2) changed
  ...
  won't have the Electracom furniture. Reports Pete wants in the ITP pack should be re-generated on
  the new build.
