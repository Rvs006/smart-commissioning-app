#!/usr/bin/env python3
"""Regression tests for the v0.1.55 evidence-validator entry point."""
...
import validate_v0155_release_evidence
...
class V0155EvidenceEntryPointTests(unittest.TestCase):
    def test_delegates_to_the_shared_contract_with_the_exact_version(self) -> None:
        arguments = ["--version", "v0.1.55"]
        with patch.object(
            validate_v0155_release_evidence.base, "main_for_version", return_value=0
        ) as delegate:
            self.assertEqual(validate_v0155_release_evidence.main(arguments), 0)
        delegate.assert_called_once_with(
            arguments,
            expected_version="v0.1.55",
            success_label="v0.1.55 Docker and release evidence: OK",
        )
    ...
    def test_dirty_portable_provenance_is_explicitly_non_publishable(self) -> None:
        ...
        manifest = (ROOT / "docs/v0.1.55-evidence-manifest.md").read_text(encoding="utf-8")
        for value in (
...
class V0155BundleProvenanceTests(unittest.TestCase):
    def _run(self, args: list[str]) -> tuple[int, object]:
        with patch.object(
            validate_v0155_release_evidence.base, "main_for_version", return_value=0
        ) as delegate:
            code = validate_v0155_release_evidence.main(args)
    ...
    def test_invalid_utf8_provenance_is_rejected_without_traceback(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            failures = validate_v0155_release_evidence._bundle_provenance_failures(
    ...
    def test_hosted_evidence_skips_the_portable_provenance_check(self) -> None:
        code, delegate = self._run(
            ["--evidence-kind", "hosted", "--version", "v0.1.55", "--commit", _CLEAN_SHA]
        )
        self.assertEqual(code, 0)
        delegate.assert_called_once()
