#!/usr/bin/env python3
"""Regression tests for the v0.1.49 evidence-validator entry point."""
...
import validate_v0149_release_evidence

ROOT = Path(__file__).resolve().parents[1]


class V0149EvidenceEntryPointTests(unittest.TestCase):
    def test_delegates_to_the_shared_contract_with_the_exact_version(self) -> None:
        arguments = ["--version", "v0.1.49"]
        with patch.object(
            validate_v0149_release_evidence.base, "main_for_version", return_value=0
        ) as delegate:
            self.assertEqual(validate_v0149_release_evidence.main(arguments), 0)
        delegate.assert_called_once_with(
            arguments,
            expected_version="v0.1.49",
            success_label="v0.1.49 Docker and release evidence: OK",
        )
    ...
    def test_dirty_portable_provenance_is_explicitly_non_publishable(self) -> None:
        ...
        manifest = (ROOT / "docs/v0.1.49-evidence-manifest.md").read_text(encoding="utf-8")
        for value in (
