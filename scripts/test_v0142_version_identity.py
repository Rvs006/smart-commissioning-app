#!/usr/bin/env python3
"""Verify release-facing version sources are v0.1.42."""
...
class V0142VersionIdentityTests(unittest.TestCase):
    def test_runtime_package_and_renderer_versions_are_v0142(self) -> None:
        ...
        for relative in ("core/pyproject.toml", "backend/pyproject.toml", "worker/pyproject.toml"):
            ...
            self.assertEqual(package["project"]["version"], "0.1.42", relative)

        frontend = json.loads((ROOT / "frontend/package.json").read_text(encoding="utf-8"))
        lock = json.loads((ROOT / "frontend/package-lock.json").read_text(encoding="utf-8"))
        self.assertEqual(frontend["version"], "0.1.42")
        self.assertEqual(lock["version"], "0.1.42")
        self.assertEqual(lock["packages"][""]["version"], "0.1.42")
        for relative in ("AGENTS.md", "CLAUDE.md"):
            self.assertIn("v0.1.42", (ROOT / relative).read_text(encoding="utf-8"), relative)
        self.assertIn("[0.1.42]", (ROOT / "CHANGELOG.md").read_text(encoding="utf-8"))
        for relative in ("AGENTS.md", "CLAUDE.md"):
            lineage = (ROOT / relative).read_text(encoding="utf-8")
            self.assertIn("operator Nmap contracts from v0.1.41", lineage, relative)
            self.assertIn("latest public release is v0.1.42", lineage, relative)
