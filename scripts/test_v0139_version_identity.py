#!/usr/bin/env python3
"""Verify release-facing version sources are v0.1.39."""
...
class V0139VersionIdentityTests(unittest.TestCase):
    def test_runtime_package_and_renderer_versions_are_v0139(self) -> None:
        ...
        for relative in ("core/pyproject.toml", "backend/pyproject.toml", "worker/pyproject.toml"):
            ...
            self.assertEqual(package["project"]["version"], "0.1.39", relative)

        frontend = json.loads((ROOT / "frontend/package.json").read_text(encoding="utf-8"))
        lock = json.loads((ROOT / "frontend/package-lock.json").read_text(encoding="utf-8"))
        self.assertEqual(frontend["version"], "0.1.39")
        self.assertEqual(lock["version"], "0.1.39")
        self.assertEqual(lock["packages"][""]["version"], "0.1.39")
