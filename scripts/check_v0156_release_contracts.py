#!/usr/bin/env python3
"""Fail-closed static contracts for the v0.1.56 release paths."""
...
def main() -> int:
    ...
    failures = base.check(
        ...
        release_notes_path="docs/release-notes-v0.1.56.md",
        migration_path="docs/migration-rollback-v0.1.56.md",
        validation_path="docs/release-validation-v0.1.56.md",
        docker_deployment_path="docs/docker-deployment-rollback-v0.1.56.md",
        evidence_validator="validate_v0156_release_evidence.py",
        evidence_test="test_v0156_release_evidence.py",
        release_contract_script="check_v0156_release_contracts.py",
    )
    required = (
        "scripts/scan_v0156_release_secrets.py",
        "scripts/test_v0156_security_scan.py",
        "scripts/test_v0156_version_identity.py",
        "docs/v0.1.56-evidence-manifest.md",
        "docs/v0.1.56-field-acceptance-checklist.md",
    )
    for relative in required:
        if not (repo / relative).is_file():
            failures.append(f"required v0.1.56 safety file is missing: {relative}")
    ...
    evidence_manifest = (repo / "docs/v0.1.56-evidence-manifest.md").read_text(encoding="utf-8")
    field_checklist = (repo / "docs/v0.1.56-field-acceptance-checklist.md").read_text(
        encoding="utf-8"
    )
    changelog = (repo / "CHANGELOG.md").read_text(encoding="utf-8")
    release_notes = (repo / "docs/release-notes-v0.1.56.md").read_text(encoding="utf-8")
    ...
    for text, label, command in (
        (release_workflow, "release workflow", "python scripts/scan_v0156_release_secrets.py --path build/release-evidence"),
        (windows_workflow, "Windows workflow", "python scripts/scan_v0156_release_secrets.py --path build/Smart_Commissioning_App_Windows_Portable"),
    ):
        if command not in text:
            failures.append(f"{label} does not execute the v0.1.56 repository secret scan")
        elif text.find("actions/upload-artifact@", text.index(command) + len(command)) < 0:
            failures.append(f"{label} does not upload an artifact after the v0.1.56 secret scan")
    zip_scan = "python scripts/scan_v0156_release_secrets.py --path $releaseZip"
    ...
    scan = "python scripts/scan_v0156_release_secrets.py --path build/release-evidence"
    if not (hosted.index("python scripts/generate_release_evidence.py") < hosted.index(scan) < hosted.index("actions/upload-artifact@")):
        failures.append("v0.1.56 evidence scan is not ordered after assembly and before upload")
    ...
    for value in ("source_commit: local-dirty-tree", "source_tree_state: dirty_local_non_publishable", "publishable: false", "clean rebuild after the authorized release"):
        if value not in evidence_manifest:
            failures.append(f"v0.1.56 evidence manifest omits provenance requirement: {value}")
    for value in (
        ...
        if value not in brief_text:
            failures.append(f"v0.1.56 Brief omits required guidance: {value}")
    for value in (
        "Run an IP discovery",
        ...
        if value not in learning_text:
            failures.append(f"v0.1.56 Learning omits required guidance: {value}")
    ...
    for value in (
        ...
        if value not in field_checklist_text:
            failures.append(f"v0.1.56 field checklist omits report evidence: {value}")
    ...
    if "scanner_raw_write" not in release_notes:
        failures.append("v0.1.56 release notes omit the Advanced-panel scanner surface")
    for value in (
        "--route '/#/brief'",
        "--route '/#/learning'",
    ...
    for value in ("2026-09-09", "latest public release is v0.1.56"):
        if value not in handoff:
            failures.append(f"v0.1.56 handoff omits: {value}")
    if failures:
        for failure in failures:
            print(f"FAIL: {failure}", file=sys.stderr)
        return 1
    print("v0.1.56 release contracts: OK")
    return 0
