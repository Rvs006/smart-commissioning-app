#!/usr/bin/env python3
"""Fail-closed real-socket v0.1.42 legacy-register capture gate.

Start an isolated anonymous Mosquitto listener first, for example:

``docker run --rm -d --name sct-v0142-mqtt -p 18883:1883 eclipse-mosquitto:2``
...
class RealMqttSocketGate(ApiTestCase):
    ...
    def _mapped_legacy_parameters(self) -> dict[str, object]:
        project_id = "v0142-socket-project"
        site_id = "v0142-socket-site"
        header = (
    ...
    def _publish_traffic(self) -> None:
        ...
        settings = MqttConnectionSettings(
            host=self.host, port=self.port, client_id="v0142-socket-publisher", timeout_seconds=5
        )
    ...
    def test_real_socket_legacy_register_deadline_and_stop_isolation(self) -> None:
        ...
        parameters.update(
            {
                ...
                "client_id": "v0142-socket-validation",
                "capture_seconds": 2,
                "use_tls": False,
        ...
        settings = MqttConnectionSettings(
            host=self.host, port=self.port, client_id="v0142-socket-stop", timeout_seconds=5
        )
