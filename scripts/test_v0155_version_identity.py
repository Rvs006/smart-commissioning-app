#!/usr/bin/env python3
"""Verify release-facing version sources and runtime boundary are v0.1.55."""
...
class V0154VersionIdentityTests(unittest.TestCase):
    def test_runtime_package_frontend_and_documentation_identity_are_v0155(self) -> None:
        source = (ROOT / "core/smart_commissioning_core/__init__.py").read_text(encoding="utf-8")
        self.assertEqual(re.findall(r'(?m)^__version__\s*=\s*["\']([^"\']+)["\']$', source), ["0.1.55"])
        for relative in ("core/pyproject.toml", "backend/pyproject.toml", "worker/pyproject.toml"):
            ...
            self.assertEqual(package["project"]["version"], "0.1.55", relative)
        frontend = json.loads((ROOT / "frontend/package.json").read_text(encoding="utf-8"))
        lock = json.loads((ROOT / "frontend/package-lock.json").read_text(encoding="utf-8"))
        self.assertEqual(frontend["version"], "0.1.55")
        self.assertEqual(lock["version"], "0.1.55")
        self.assertEqual(lock["packages"][""]["version"], "0.1.55")
        agents = (ROOT / "AGENTS.md").read_bytes()
        self.assertEqual(agents, (ROOT / "CLAUDE.md").read_bytes())
        lineage = " ".join(agents.decode("utf-8").split())
        self.assertIn("the latest public release is v0.1.55", lineage.lower())
        self.assertIn("2026-08-27", lineage)
        self.assertIn("[0.1.55]", (ROOT / "CHANGELOG.md").read_text(encoding="utf-8"))
    ...
    def test_docker_defaults_use_the_canonical_release_stamp(self) -> None:
        for relative in ("backend/Dockerfile", "worker/Dockerfile", "frontend/Dockerfile"):
            self.assertIn("ARG APP_VERSION=v0.1.55", (ROOT / relative).read_text(encoding="utf-8"), relative)
        for relative in ("infra/docker-compose.yml", "infra/docker-compose.sync-acceptance.yml"):
            self.assertIn('APP_VERSION: "${APP_VERSION:-v0.1.55}"', (ROOT / relative).read_text(encoding="utf-8"), relative)
